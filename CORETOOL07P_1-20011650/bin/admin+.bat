...
REM
REM This script is used to launch the Administration Tool of SAP Convergent Charging.
REM
...
SET HIGHDEALCLASSES=%HIGHDEAL_LIB%/core_admin.jar;%HIGHDEAL_LIB%/logging.jar;%HIGHDEAL_LIB%/sap.com~tc~logging~java.jar;%HIGHDEAL_LIB%/core_chargingplan.jar;%HIGHDEAL_LIB%/core_chargingprocess.jar;%HIGHDEAL_LIB%/core_client.jar;%HIGHDEAL_LIB%/common_message.jar;%HIGHDEAL_LIB%/common_util.jar

SET CLASSPATH=%HIGHDEALCLASSES%

SET JAVA=%SAPCC_JAVA_HOME%/bin/java

REM change this variable if you want to select another language
SET LANGUAGE=-Duser.country=US -Duser.language=en

"%JAVA%" -Dfile.encoding=UTF-8 -classpath %CLASSPATH% %LANGUAGE% com.highdeal.admin.plus.AdminPlus %1 %2 %3 %4 %5 %6 %7 %8 %9
endLocal
GOTO end
